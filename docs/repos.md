...
The two Rust workspace members are `crates/server` and `crates/clumsiesd`. Swift owns the native interface. Bun runs the VitePress documentation tooling.
...
| `crates/clumsiesd/src/agent_runtime/` | MCP contract and short-lived agent proxies | Understand the agent-facing tool boundary |
| `crates/clumsiesd/src/state.rs`, `draft.rs` | Local state, durable Draft writes, and synchronization | Understand what “queued” means |
| `crates/clumsiesd/src/commit_sync.rs`, `project_storage.rs` | Commit installation, local generations, and cache locations | Trace published data reaching a Mac |
| `crates/clumsiesd/src/search/` | Effective Memory, chunking, indexing, and retrieval | Understand how relevant fragments are selected |
...
Paths in the first row are relative to `apps/macos/Sources/`; the other daemon paths are relative to `crates/clumsiesd/src/`.
...
| Draft upload, merge, projection updates, and two-daemon convergence | `crates/clumsiesd/tests/server_integration.rs` |
| Local persistence and process restart | `crates/clumsiesd/tests/daemon_lifecycle.rs` |
| Agent proxy and real XPC boundary | `crates/clumsiesd/tests/agent_runtime_xpc_e2e.rs` |
...
[Desktop workspace](https://github.com/lilhammerfun/clumsies/blob/5d038ffb0ad6e170680618a8fcd0e1ff3d760f77/apps/macos/Sources/Domain/WorkspaceStore.swift) · [MCP contract](https://github.com/lilhammerfun/clumsies/blob/5d038ffb0ad6e170680618a8fcd0e1ff3d760f77/crates/clumsiesd/src/agent_runtime/mcp_contract.rs) · [daemon state](https://github.com/lilhammerfun/clumsies/blob/5d038ffb0ad6e170680618a8fcd0e1ff3d760f77/crates/clumsiesd/src/state.rs) · [Server router](https://github.com/lilhammerfun/clumsies/blob/5d038ffb0ad6e170680618a8fcd0e1ff3d760f77/crates/server/src/http.rs) · [Review transactions](https://github.com/lilhammerfun/clumsies/blob/5d038ffb0ad6e170680618a8fcd0e1ff3d760f77/crates/server/src/changes/postgres.rs)
