...
| 3. Editing, sync, and retrieval | `crates/clumsiesd/src/agent_runtime/`, `draft.rs`, `commit_sync.rs`, `search/`; macOS Memory and Review features/services | Project editing defaults to Project scope, including adaptations. Store remains a Draft operation. Two members converge after merge; clean reconciliation, conflicts, offline retry, approval invalidation, and matching index versions are verified. |
