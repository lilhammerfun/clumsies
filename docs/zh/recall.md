...
| 关注点 | 路径 |
|---|---|
| 共享投影、绑定过滤与 DSH reader | `crates/clumsiesd/src/recall.rs` |
| 摘要快照与任务分页 | `crates/clumsiesd/src/recall/paging.rs` |
| Codex rollout discovery 与 parser | `crates/clumsiesd/src/recall/codex.rs` |
| 历史片段与 Project 边界 | `crates/clumsiesd/src/retrieval_history.rs` |
