---
description: 在 App 中提交 Codex 修改后的草稿，由人检查差异并发布到组织 Memory。
prev:
...
上一页让 Codex 为 `deployment-rollback.md` 补充了回滚后的验证要求。这一页把草稿提交为 **Review**，由人检查差异，再发布到组织的共享 Memory。
...
- 由组织 **owner/admin** 完成发布。Payments 的 Project admin 可以管理项目，却不会仅凭这个角色获得组织内容的发布权限。
...
确认可以发布后，组织 owner/admin 点击工具栏的勾选按钮，提示文字为 **Approve and merge this Review**；也可以使用 macOS 菜单 **Review → Approve**。

**当前 App 中的 Approve 会直接执行批准并合并。** 成功后状态变为 **Merged**，本次修改进入组织正式内容。如果你没有组织发布权限，请由有权限的同事完成这一步。仅保存草稿、提交 Review 或在项目中读到新句子，都不能证明已经发布。

## 确认完成

1. Review 状态显示 **Merged**。
2. 回到 **Memory**，将筛选器切换为 **Org**，打开 `deployment-rollback.md`，确认已发布正文包含新增句子。
...
先核对前两项的 **Merged** 和 **Org** 正式正文，再进行 Codex 验证。项目草稿在发布前也可能被检索到，所以仅命中新句子不能证明它已经发布。

以后选择这份组织 Memory 的项目会使用发布后的共享内容；这次修改不需要在 Payments 中重新执行 Add to Project。
...
- **批准按钮不可用**：确认组织发布权限、详情加载状态，以及是否需要先处理最新共享变更。
