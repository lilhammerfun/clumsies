...
Clumsies 有四类接口边界，分别服务于不同调用方。即使它们参与同一次操作，读取 Memory、编辑本地 Draft 和发布 Project 或 Organization Memory，也仍是不同的能力。
...
**Public API 指产品接口，并不表示匿名可访问。** 常规 Public 和 Admin 请求都使用 `Authorization: Bearer …`。组织管理要求 Organization 的 `owner` 或 `admin` 角色，项目管理路由也接受相应的 Project 角色。发布按 Review 目标检查权限：Project Memory 由 Project owner/admin 发布，Org Memory 由 Organization owner/admin 发布。
...
这个领域回答：**谁在调用，他能做哪些事？** Project 是协作和 Memory 选择的边界；Project 与 Organization 分别拥有各自的正式 Memory。
...
| 创建 Project | `POST /api/v1/projects` | 已登录的组织成员；要求 `Idempotency-Key`；创建者成为 Project owner |
...
这个领域回答：**有哪些正式资源，这个 Project 使用哪些资源？** Memory 有稳定身份和 Markdown 正文。选择集合保存资源 ID，不会把正文复制成另一份独立权威内容。

| 能力 | HTTP 操作 | 结果或约束 |
| --- | --- | --- |
| 浏览已发布的 Organization Memory | `GET /api/v1/org/memories` 及其 `/{memory_id}` | 元数据列表或完整资源 |
| 读取已发布的 Project 自有 Memory | `GET /api/v1/projects/{project_id}/memories` 及其 `/{memory_id}` | 项目自有的 `scope=project` 资源；这两个路由不返回已选 Organization Memory 的投影 |
...
这个领域回答：**提出了哪些变更，它们是否已经到达 Server？** Draft 由 Project 携带，每份 Draft 的发布目标是该 Project 或 Organization。本地保存、同步成功和正式发布是三个独立阶段。
...
| 自动协调干净 Draft | `POST /api/v1/drafts/{draft_id}/auto-rebases` | 仅作者可调用；预期 Draft 版本；冲突保留基线并通知作者 |
...
这个领域回答：**哪些提案正在被审阅，谁有权发布？** 一个 Review 可以包含多个 Draft，也可以一次提交上百个文件，但必须属于同一个发布目标。

| 能力 | HTTP 操作 | 并发与权限 |
| --- | --- | --- |
| 提交 Draft | `POST /api/v1/reviews` | 同一个 Project、相同 scope 下，当前作者拥有的 open Draft；每个 Draft 版本 + 权威 Ref `If-Match` |
...
| 记录审批结论 | `POST /api/v1/reviews/{review_id}/decisions` | 发布所有者的 owner/admin；`approved` 或 `rejected`，预期 Review 版本 |
| 批准并发布 | `POST /api/v1/reviews/{review_id}/merges` | 发布所有者的 owner/admin；预期 Review 版本 + 权威 Ref `If-Match` |
...
Project Review 可显式携带 `org_contribution` 选择。Project 合并后，Server 从该固定 Commit 创建独立 Org Review；`POST /api/v1/reviews/{review_id}/org-contribution` 可幂等重试创建。创建失败或 Org Review 被拒绝都不撤销项目发布。
...
| 查看历史 | `GET /api/v1/org/commits` 或 `GET /api/v1/projects/{project_id}/commits` | Organization 历史或包含项目发布与选择变化的 Project 历史 |
