...
在 **Clumsies → 设置 → 通用 → 语言**中，选择**跟随系统**、**English** 或
**简体中文**。这个选择只影响 Clumsies，系统是英文时也可以单独使用中文版。
点击**重启并应用**，Clumsies 会先保存待处理的更改，再自动重新打开当前 App，
所有窗口和菜单都会使用所选语言。如果选择继续编辑，或保存失败，本次重启会取消。
也可以选择**稍后**，下次启动时生效。文档正文、项目名称和文件路径保留原有内容。
