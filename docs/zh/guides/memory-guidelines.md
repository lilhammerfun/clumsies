...
2. 点击 **Preview guidelines and their sources** 查看完整英文模板及研究出处；也可以直接点击 **Use Default Guidelines**。
3. Clumsies 创建并打开 `CLUMSIES.md` 草稿。以后需要调整约定时，切换到 **Source** 编辑即可。

草稿在发布前已参与当前项目的有效记忆。其他项目需要共用时，按现有流程[审阅并发布](/zh/quickstart/review-and-publish)，再在那些项目中选择同一资源。

采用默认规范是可选操作。你也可以通过 **Create a Memory** 或 **File → New Memory** 创建自己的内容。

## 沿用已有规范

初始化前，Clumsies 会检查配置路径、当前项目草稿和组织记忆，保留已有规范。组织已有对应文档时，**Use Organization Guidelines** 会将同一资源加入当前项目的选择，不复制一份新文档；这个操作需要项目管理员权限。
