...
作者点击工具栏的 **Update Review** 图标，打开独立的原生窗口，集中处理整个 Review 的落后文件。
窗口带关闭、最小化、缩放及全屏控件，主窗口里的详情页保持可用。文件切换在同一个更新窗口内完成；
比较区和结果区分别滚动，可拖动分隔线调整空间。关闭、退出登录和退出应用都会确认未保存编辑，
保存期间阻止关闭。应用后刷新 Review 详情。图标带 tooltip 和 accessibility label；非作者看到等待作者更新的说明。

Remote 与 Draft 的冲突片段并排显示，下方展示合并结果。界面不显示 Resolve 页签或字段级
冲突计数。选择片段只修改对应位置，保留双方其他可自动合并的改动；正文冲突处理完后，
可继续编辑合并结果。完整版本替换放在次要菜单内，并明确确认会替换整份结果。

1. 一次为全部落后文件生成同一共享版本下的候选，列表区分自动合并、待解决、已解决。
2. 路径冲突显示双方路径并允许输入最终路径；删除冲突明确选择保留哪份文件或保持删除。
3. 选择完成、路径有效且无生成的冲突标记后自动就绪，不再额外点击 Mark Resolved。
   切换文件保留选择和编辑。
4. **Save All Drafts** 提交完整有序集合。Server 在一个事务里校验成员、Review/Draft
