...
2. 有冲突时并排显示 Remote 和 Draft 各自相对共同原版的 Diff，选择按钮与标题同行。
   ...
   路径占用时可输入自定义路径。
3. 选择完成后直接显示“最新 Remote → 更新后 Draft”的普通 Diff。重置图标允许重新选择。
4. 自动合并文件沿用普通 Diff，文件树的文件名旁用蓝底 **Auto-rebased** 标签替代同步图标，
   仅在候选有效且无冲突时显示；tooltip 说明点击工具栏保存后才正式应用。
   冲突文件用橙底 **Conflict** 标签，当前无需更新的文件不加标记。Diff 上方不再单独显示状态行，
   不显示文件数量统计或额外结果区。
5. 工具栏 **Save Review Updates** 提交完整有序集合，所有冲突处理完之前禁用。
...
保存更新与决策按钮只属于已打开的详情，使用图标、tooltip 和无障碍名称。Save Review Updates
使用 square.and.arrow.down，仅作者有待处理候选时显示；加载、保存及仍有未解决冲突时禁用。
