...
final class AdministrationLoadingTests: XCTestCase {
    func testEachPageRequestsOnlyItsOwnResourcesAndStopsAtOnePage() async throws {
        for (section, expectedPaths) in [
            ...
            let page = try await AdministrationModel.loadAdministrationPage(section: section) { path, query in
    ...
    func testProjectHistoryLoadsOneProjectByIDWithoutLoadingTheDirectory() async throws {
        ...
        let result = try await AdministrationModel.fetchAdministrationProject(id: expected.id) { path in
    ...
    func testProjectHistoryReportsRequestFailureAndRejectsCancelledResponses() async throws {
        for status in [404, 500] {
            do {
                _ = try await AdministrationModel.fetchAdministrationProject(id: "off-page") { _ in
        ...
        let task = Task.detached {
            ...
            return try await AdministrationModel.fetchAdministrationProject(id: "off-page") { _ in
    ...
    func testSearchQueryPersistsAcrossPagesAndEmptyQueryIsOmitted() async throws {
        for section: AdministrationSection in [.members, .audit] {
            ...
            for cursor: String? in [nil, "cursor & 2"] {
                _ = try await AdministrationModel.loadAdministrationPage(
            ...
            _ = try await AdministrationModel.loadAdministrationPage(section: section, query: "  ") { path, query in
                await requests.record(path: path, query: query)
                return Self.response(path: path)
            }
    ...
    func testStaleIdentityProviderMakesAccessReadOnlyWithoutAffectingOrganization() async throws {
        let access = try await AdministrationModel.loadAdministrationPage(section: .access) { path, _ in
            Self.response(path: path, stale: path.hasSuffix("identity-provider"))
        }
        let organization = try await AdministrationModel.loadAdministrationPage(section: .organization) { path, _ in
    ...
    func testInvalidPaginationAndFailedRequestsDoNotProduceAPage() async throws {
        for nextCursor in ["", "same", "earlier"] {
            do {
                _ = try await AdministrationModel.loadAdministrationPage(section: .audit, cursor: "same", seenCursors: ["earlier"]) { path, _ in
        ...
        do {
            _ = try await AdministrationModel.loadAdministrationPage(section: .members) { _, _ in
    ...
    func testCancelledSearchDiscardsALateResponseAfterANewQueryCompletes() async throws {
        ...
        let oldSearch = Task {
            try await AdministrationModel.loadAdministrationPage(section: .members, query: "old") { path, _ in
        ...
        oldSearch.cancel()
        let current = try await AdministrationModel.loadAdministrationPage(section: .members, query: "new") { path, _ in
    ...
    func testApplyingMemberPagesPreservesOtherPagesAndAReplacementDropsPreviousResults() async throws {
        ...
        let first = try await AdministrationModel.loadAdministrationPage(section: .members) { path, _ in
            Self.response(path: path, memberId: "first")
        }
        let second = try await AdministrationModel.loadAdministrationPage(section: .members, cursor: "next") { path, _ in
    ...
    func testAuthorityResetClearsPagesAndRejectsAnInFlightResponse() async {
        ...
        let model = AdministrationModel(context: workspace.context, onWorkspaceChanged: {}) { path, query in
            ...
            return Self.response(path: path, memberId: "member")
        }
        await model.load(section: .members)
        XCTAssertEqual(model.snapshot?.members.map(\.id), ["member"])

        let pending = Task { await model.load(section: .members, query: "slow") }
        await gate.waitUntilRequested()
    ...
    func testSameAuthorityReloadPreservesPagesButPermissionLossClearsThem() async {
        ...
        workspace.apply(Self.workspaceSnapshot())
        XCTAssertEqual(model.snapshot?.members.map(\.id), ["member"])
        workspace.apply(Self.workspaceSnapshot(capabilities: []))
        XCTAssertNil(model.snapshot)
        XCTAssertTrue(model.pageStates.isEmpty)
    ...
    private static func workspaceSnapshot(capabilities: Set<String> = ["admin:write"]) -> WorkspaceSnapshot {
        .init(
            ...
            organization: .init(orgId: "org", name: "Example"),
            capabilities: capabilities, projects: [], activeProjectId: nil,
            orgRefCommitId: nil, orgRefEtag: "", resources: [],
            runtime: .init(
                health: .init(
                    daemonVersion: "test", agentRuntime: .init(protocolRevision: 1, buildId: "test"),
                    serverUrl: "https://example.com", projectId: nil, daemonInstallationId: "test",
