...
final class DaemonContractTests: XCTestCase {
    ...
    func testXPCTransportForwardsTaskCancellation() throws {
        let macOSRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let source = try String(
            contentsOf: macOSRoot.appending(path: "Sources/Services/Daemon/DaemonXPCClient.swift"),
            encoding: .utf8
        )
    ...
    func testAvatarLoadingIsCachedAndCoalesced() throws {
        let macOSRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let source = try String(
            contentsOf: macOSRoot.appending(path: "Sources/Libraries/UI/AvatarView.swift"),
            encoding: .utf8
        )
