...
final class RecallLoadingTests: XCTestCase {
    ...
    func testProjectChangesIgnoreLateResultsAndDuplicateLoads() async throws {
        ...
        var requests: [String] = []
        let model = RecallModel(daemon: DaemonXPCClient(serviceName: "test.unused"), fetchSessions: { request in
    ...
    func testFailedActivityRefreshRetainsContentAndCanRetry() async {
        ...
        let model = RecallModel(daemon: DaemonXPCClient(serviceName: "test.unused"), fetchSessions: { _ in
            attempt += 1
    ...
    func testListLoadsNextPageOnlyWhenItsFooterBecomesVisible() async throws {
        ...
        await model.load()
        let hosting = NSHostingView(rootView: RecallSessionList(model: model))
    ...
    func testRestoresLastProjectBeforeLoadingAndFallsBackWhenUnavailable() async throws {
        let suite = "RecallLoadingTests.\(UUID().uuidString)"
        let defaults = try XCTUnwrap(UserDefaults(suiteName: suite))
        defer { defaults.removePersistentDomain(forName: suite) }
        var requests: [String?] = []
        func makeModel() -> RecallModel {
            RecallModel(daemon: DaemonXPCClient(serviceName: "test.unused"), defaults: defaults, fetchSessions: { request in
