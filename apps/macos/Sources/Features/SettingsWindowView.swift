...
struct SettingsWindowView: View {
            ...
            Group {
                ...
                .toolbar {
                    ...
                    if case .organization(let section) = navigation.destination {
                        ToolbarItem {
                            Button {
                                Task { await store.loadAdministration(section: section, force: true) }
                            } label: {
                                Image(systemName: "arrow.clockwise")
                            }
                            .disabled(navigation.hasUnsavedChanges || store.isMutatingAdministration
                                || store.administrationState(for: section).isLoading)
    ...
    private var organizationLanding: some View {
        ...
        .formStyle(.grouped)
        .task { await store.loadAdministration(section: .organization) }
    }
