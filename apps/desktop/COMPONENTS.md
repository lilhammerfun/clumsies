...
| `ClassicSearchField` (5 files) | The one search box, with a focus token driven by Cmd-K. | Library. `input::Input` for the field; `command` is the palette Cmd-K should open. Not built yet. |
...
| `PathTreeView`, `PathTreeRowLabel`, `FileSymbolView` (2 files) | A path-derived tree, one row shape, file-type icons. | Ours: `components/memory_tree.rs` builds the tree from paths; the library's `tree` draws it. Multi-select, a context menu and inline rename are not built. |
...
| `FileTreeView` (1 screen) | The Memory navigator: multi-select, draft-coloured titles, rename, context menu. | Ours, partly. Single selection works, one click both selects and expands (a known gap), a draft is marked with a label rather than a colour. |
...
| `ReviewRequestSheet` (3 files) | Title, description, batch confirmation, conflict steps. | Ours: `document::ReviewDialog` covers the single-draft case; candidates and conflicts are unbuilt. |
