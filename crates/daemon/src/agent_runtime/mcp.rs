...
use super::mcp_contract::{
    DEFAULT_GUIDELINES_PATH, decode_tool_call_params, memory_guidelines_instructions,
    parse_tool_call, tool_definitions_with_guidelines,
};
...
/// Describe Clumsies memory routing, retrieval, and authorized proposal writes.
pub fn server_instructions(guidelines_path: &str) -> String {
    let guidelines = memory_guidelines_instructions(guidelines_path);
    format!(
        "Consult Clumsies project memory for project questions, analysis, planning, implementation, debugging, and review in this bound workspace, even when the user does not mention Clumsies or memory. Follow applicable host memory policies and additionally query Clumsies, even when host memory has already been consulted. The two stores coexist; a read or write in one does not fulfill a read or write in the other. Respect explicit user requests to skip Clumsies or use only another source or destination; do not route personal or cross-project preferences here unless requested. Before project analysis, planning, or editing, call memory with op: {{ activate: {{ query: \"...\" }} }}; reuse an activation already made for the same task. It returns ranked memory fragments ready for the current reasoning context. Pass its next_state only while the earlier fragments remain in the model context; omit state after context compaction or when starting fresh. Use memory with op: {{ load: {{ ids: [...] }} }} to read complete resources by known id or path. {guidelines} Use memory with op: {{ store: ... }} only when the user explicitly requests memory maintenance, such as remembering, recording, correcting, updating, or deleting project knowledge; ordinary development and current-task reminders do not authorize Clumsies memory writes. Load any existing target before changing it. Store creates a Project-owned proposal in the bound Project and changes only that Project's pre-merge Effective Memory overlay; it cannot decide a Review, merge, or publish Organization authority. Report the saved resource and Draft status. If Clumsies is unavailable or unbound, report that instead of silently substituting host-local memory or claiming success."
    )
}
